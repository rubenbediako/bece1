...
class WhatsAppCodeService {
  ...
  private adminPhoneNumber = '233540456414'; // 0540456414 with Ghana country code
  ...
  private notifyAdmin(request: CodeRequest): void {
    ...
    // Optional: Open admin notification (can be disabled in production)
    if (typeof window !== 'undefined' && Math.random() > 0.7) { // Only show sometimes to avoid spam
      window.open(adminWhatsAppUrl, '_blank');
    }
  }
